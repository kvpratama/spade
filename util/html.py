...
class HTML:
    def __init__(self, web_dir, title, start_time, refresh=0):
        ...
        with self.doc:
            h1("Experiment start time: {}".format(start_time))
            h1("Current time: {}".format(datetime.datetime.now().strftime("%I:%M%p on %B %d, %Y")))
        if refresh > 0:
            with self.doc.head:
